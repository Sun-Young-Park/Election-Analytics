...
pvstate_df$region <- tolower(pvstate_df$state)
...
## map: GOP pv2p
ggplot(pv_map, aes(long, lat, group = group))+
...
pvstate_df %>%
    ...
    left_join(states_map, by = "region") %>%
    mutate(winner = ifelse(R > D, "republican", "democrat")) %>%
    ## can pipe (%>%) data directly into `ggplot()`!
    ggplot(aes(long, lat, group = group)) +
    facet_wrap(facets = year ~.) + ## specify a grid by year
    geom_polygon(aes(fill = winner), color = "white") +
    scale_fill_manual(values = c("blue", "red")) +
    theme_void() +
    ggtitle("Presidential Vote Share by State (1980-2016)") + 
